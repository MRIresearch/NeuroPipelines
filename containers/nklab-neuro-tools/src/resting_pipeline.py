...
class RestPipe:
    ...
    def step2(self):
        ...
        if os.path.isfile(newfile + ".nii.gz") and os.path.isfile(newfile + ".par"):
            ...
            data1 = data.get_data()
            if not (data1.dtype.name == 'float32' or data1.dtype.name == 'float64'):
                data1 = data1.astype(dtype=np.float64)
    ...
    def step6(self):
        ...
        time_subTR = time_all[0:-1:long(self.tr_ms)]
        ...
        tmp[long(len1-tmpMA+tmpMA2-1):long(len1+tmpMA2)]=tmpAB
        tmp[long(len2-tmpMA2-1):long(len2+tmpMA-tmpMA2)]=tmpBA
