...
def test_dcm(dcmfile):
	print(mp.current_process())
	logtext(None, "Processing DICOM : " + dcmfile)

	subid=dcmfile.split("^^")[1]
	session=dcmfile.split("^^")[2]
	dcmfile=dcmfile.split("^^")[0]
	subnum = ''.join([i for i in subid if i.isdigit()])
	
	logtext(None,"Project:{PROJECT} Subject:{subnum} Session:{session}".format(PROJECT=PROJECT, subnum=subnum, session=session))


def send_dcm(dcmfile):
	logtext(None, "Processing DICOM : " + dcmfile)

	subid=dcmfile.split("^^")[1]
	session=dcmfile.split("^^")[2]
	dcmdfile=dcmfile.split("^^")[0]
	ds = dcmread(dcmfile)
	subnum = ''.join([i for i in subid if i.isdigit()])
	...
	for subject in subjects:
		...
		for session in os.listdir(subject):
			...
			for dicomdir in os.listdir(subject + '/' + session):
				...
				#split up dcm files into pool
				procs=mp.cpu_count()
				pool = mp.Pool(procs)
				loops = math.ceil(len(dcmfiles)/procs)
				dcmbatch=[]

				for count in range(len(dcmfiles)):
					dcmbatch.append(dcmfiles[count]+'^^'+subid+"^^"+session)
				...
				#	for p in range(procs):
				#		if (count + 1) > len(dcmfiles):
				#			break
				#		else:
				#			dcmbatch[p].append(dcmfiles[count]+'^^'+subid+"^^"+session)
				#			count=count+1

				pool.map(test_dcm,(dcmbatch))
				#pool.map(test_dcm,(dcmfiles, PROJECT,subid, session, ae,AEIP,AEPORT,AETITLE, LOGFILE ))

				#for p in range(procs):
				#	pool.map(test_dcm,(dcmbatch[p], PROJECT,subid, session, ae,AEIP,AEPORT,AETITLE, LOGFILE ))
				#for dcmfile in dcmfiles:
				#	send_dcm(dcmfile, PROJECT,subid, session, ae,AEIP,AEPORT,AETITLE, LOGFILE )
